...
The go-zh-fhir library currently provides comprehensive DICOM and DIMSE networking capabilities but lacks user-facing CLI
...
patterns for CLI structure, build info management, and user experience that should be adapted for go-zh-fhir.

## What Changes

This proposal adds a comprehensive DICOM utility CLI as a subcommand of the main `zh-fhir` CLI tool. The implementation
will provide six core commands with a consistent, user-friendly interface built using modern TUI libraries.

### New CLI Commands

1. **zh-fhir dicom cecho** - DICOM verification service (C-ECHO)
   - Test network connectivity to DICOM servers
   - Verify association negotiation
   - Support for custom AE titles, timeouts

2. **zh-fhir dicom cstore** - DICOM storage service (C-STORE SCU)
...
3. **zh-fhir dicom dump** - Inspect DICOM file contents
...
4. **zh-fhir dicom modify** - Modify DICOM file tags
   - Insert/update DICOM tags
   - Delete DICOM tags
   - Regenerate UIDs (Study, Series, Instance)
   - Batch processing support
   - Inspired by DCMTK dcmodify but with idiomatic Go flags
   - Native Go implementation using go-zh-fhir dicom package

5. **zh-fhir dicom scp** - Run DICOM SCP server
...
6. **zh-fhir dicom organize** - Reorganize DICOM directory structure
...
- **Native Implementation**: Use go-zh-fhir dicom and dimse packages directly
...
**New Files**:
- `cmd/zh-fhir/main.go` - CLI entry point with version injection
- `cmd/zh-fhir/internal/build/info.go` - Build information management
- `cmd/zh-fhir/internal/cli/cli.go` - CLI setup with Kong
- `cmd/zh-fhir/internal/config/config.go` - Global configuration context
- `cmd/zh-fhir/internal/dicom/commands/` - Individual command implementations:
...
- `cmd/zh-fhir/internal/dicom/ui/` - TUI components:
...
- ✅ **Integration Ready**: Natural integration point for go-zh-fhir library users
